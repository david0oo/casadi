...
namespace casadi {
  ...
  int CASADI_NLPSOL_TOLERANCETUBEMETHOD_EXPORT
    ...
    plugin->creator = ToleranceTubeMethod::creator;
    plugin->name = "tolerancetubemethod";
    plugin->doc = ToleranceTubeMethod::meta_doc.c_str();
    plugin->version = CASADI_VERSION;
    plugin->options = &ToleranceTubeMethod::options_;
    plugin->deserialize = &ToleranceTubeMethod::deserialize;
    return 0;
  }

  extern "C"
  void CASADI_NLPSOL_TOLERANCETUBEMETHOD_EXPORT casadi_load_nlpsol_tolerancetubemethod() {
  ...
  const Options ToleranceTubeMethod::options_
  = {{&Nlpsol::options_},
     {{"solve_type",
       {OT_STRING,
      ...
      {"tolerance_tube_beta",
       {OT_DOUBLE,
        "Tolerance-Tube beta. Reduction factor for tolerance tube."}},
      {"tr_rad0",
       {OT_DOUBLE,
  ...
  void ToleranceTubeMethod::init(const Dict& opts) {
    ...
    optimality_tol_ = 1e-7;
    feasibility_tol_ = 1e-7;
    tolerance_tube_tol_ = 1e-3;
    tolerance_tube_beta_ = 1.0;
    tr_eta1_ = 0.25;
    tr_eta2_ = 0.75;
    tr_alpha1_ = 0.5;
    tr_alpha2_ = 2.0;
    ...
    for (auto&& op : opts) {
      ...
      } else if (op.first == "tolerance_tube_beta") {
        tolerance_tube_beta_ = op.second;
    ...
    casadi_assert(!qpsol_plugin.empty(), "'qpsol' option has not been set");
    qpsol_restoration_ = conic("qpsol_restoration", qpsol_plugin, {{"a", Asp_restoration}},
                  qpsol_options);
    ...
    set_tolerancetubemethod_prob();
    // Allocate memory
    casadi_int sz_w, sz_iw;
    casadi_tolerancetubemethod_work(&p_, &sz_iw, &sz_w, sz_anderson_memory_);
    alloc_iw(sz_iw, true);
    alloc_w(sz_w, true);
    if (convexify_) {
  ...
  void ToleranceTubeMethod::set_tolerancetubemethod_prob() {
  ...
  void ToleranceTubeMethod::set_work(void* mem, const double**& arg, double**& res,
                                casadi_int*& iw, double*& w) const {
    auto m = static_cast<ToleranceTubeMethodMemory*>(mem);

    // Set work in base classes
    Nlpsol::set_work(mem, arg, res, iw, w);

    m->d.prob = &p_;
    casadi_tolerancetubemethod_init(&m->d, &iw, &w, sz_anderson_memory_);

    m->iter_count = -1;
  }

  int ToleranceTubeMethod::init_mem(void* mem) const {
...
double ToleranceTubeMethod::eval_m_k(void* mem) const {
  auto m = static_cast<ToleranceTubeMethodMemory*>(mem);
  auto d = &m->d;
  if (use_sqp_) {
...
double ToleranceTubeMethod::eval_tr_ratio(double val_f, double val_f_corr, double val_m_k) const {
  // uout() << "predicted reduction is here:" << -val_m_k << std::endl;
  return (val_f - val_f_corr) / (-val_m_k);
}

int ToleranceTubeMethod::eval_switching_condition(double current_infeasibility, double val_m_k) const {
  // uout() << "pred: " << (-val_m_k) << std::endl;
  if (-val_m_k >= 0.1 * current_infeasibility){
...
void ToleranceTubeMethod::tr_update(void* mem, double& tr_rad, double tr_ratio, std::string phase) const {
  auto m = static_cast<ToleranceTubeMethodMemory*>(mem);
  auto d = &m->d;
...
int ToleranceTubeMethod::step_update(void* mem, double tr_ratio) const {
  auto m = static_cast<ToleranceTubeMethodMemory*>(mem);
  auto d_nlp = &m->d_nlp;
  auto d = &m->d;
...
*/
void ToleranceTubeMethod::anderson_acc_step_update(void* mem, casadi_int iter_index) const {
  auto m = static_cast<ToleranceTubeMethodMemory*>(mem);
  auto d = &m->d;
...
*/
void ToleranceTubeMethod::anderson_acc_init_memory(void* mem, double* step, double* iterate) const {
  auto m = static_cast<ToleranceTubeMethodMemory*>(mem);
  auto d = &m->d;
...
*/
void ToleranceTubeMethod::anderson_acc_update_memory(void* mem, double* step, double* iterate) const {
  auto m = static_cast<ToleranceTubeMethodMemory*>(mem);
  auto d = &m->d;
...
*/
int ToleranceTubeMethod::feasibility_iterations(void* mem, double tr_rad, double tube_size) const {
  auto m = static_cast<ToleranceTubeMethodMemory*>(mem);
  auto d_nlp = &m->d_nlp;
  auto d = &m->d;
...
//   accumulated_as_ex = 0
  ...
  for (int j=0; j<max_inner_iter_; ++j) {
    ...
    if (inner_iter % watchdog_ == 0) {
      ...
      if (current_infeasibility <= tolerance_tube_beta_*tube_size && projection_ratio < 0.5) {
        // kappa_acceptance = true;
        return 0;
      }
...
void ToleranceTubeMethod::evaluate_f(ToleranceTubeMethodMemory* m, double* input_z, const double* parameter, double& output) const{
  // Evaluate f
  m->arg[0] = input_z;
  m->arg[1] = parameter;
  m->res[0] = &output;
...
void ToleranceTubeMethod::evaluate_g(ToleranceTubeMethodMemory* m, double* input_z, const double* parameter, double* output) const{
  // Evaluate f
  m->arg[0] = input_z;
  m->arg[1] = parameter;
  m->res[0] = output;
...
int ToleranceTubeMethod::evaluate_grad_f_jac_g(ToleranceTubeMethodMemory* m, double* input_z, const double* parameter, double* output_gradient, double* output_jacobian) const {
  m->arg[0] = input_z;
  m->arg[1] = parameter;
...
int ToleranceTubeMethod::solve(void* mem) const {
    auto m = static_cast<ToleranceTubeMethodMemory*>(mem);
    auto d_nlp = &m->d_nlp;
    auto d = &m->d;
    ...
    double tube_size = tolerance_tube_tol_;
    ...
    while (true) {
      ...
      if (print_iteration_) {
        ...
        print_iteration(m->iter_count, d_nlp->objective, m_k, tr_ratio,
                        m->primal_infeasibility, m->dual_infeasibility, dx_norminf, m->reg, tr_rad_prev, info);
        info = "";
      }
      tr_rad_prev = tr_rad;
      ...
      //%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
      // Formulate the QP and try to solve it
      //%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
      // Define lower bounds
      ...
      if (ret != 0){
        ...
        if (m->primal_infeasibility <= feasibility_tol_) {
        if (print_status_) {
        ...
        double *temp1, *temp2;
        casadi_copy(d->Jk, Asp_.nnz(), d->Jk_restoration);
        temp1 = d->Jk_restoration + Asp_.nnz();
        casadi_fill(temp1, ng_, -1.);
        temp1 += ng_;
        casadi_fill(temp1, ng_, 1.);

        // Initialize bounds
        casadi_copy(d->lbdz, nx_+ng_, d->lbdz_restoration);
        temp1 = d->lbdz_restoration + nx_;
        temp2 = d->lbdz_restoration + nx_+2*ng_;
        casadi_copy(temp1, ng_, temp2); // shift the bounds for constraints
        casadi_clear(temp1, 2*ng_);

        casadi_copy(d->ubdz, nx_+ng_, d->ubdz_restoration);
        temp1 = d->ubdz_restoration + nx_;
        temp2 = d->ubdz_restoration + nx_+2*ng_;
        casadi_copy(temp1, ng_, temp2); // shift the bounds for constraints
        ...
        casadi_clear(d->z_feas, nx_ + ng_);
        double* temp_mem = d->z_feas+nx_;
        casadi_mv(d->Jk, Asp_, d->dx_restoration, temp_mem, false); // problem was dx had inf values because infeasible!
        for (casadi_int i = 0; i < ng_; ++i) {
          if (d->ubdz_restoration[nx_+2*ng_+i]-temp_mem[i] < 0) {
            d->dx_restoration[nx_+i] = fmax(-d->ubdz_restoration[nx_+2*ng_+i]+temp_mem[i], 0);
          }

          if (d->lbdz[nx_+2*ng_+i]-temp_mem[i] > 0) {
            d->dx_restoration[nx_+ng_+i] = fmax(d->lbdz_restoration[nx_+2*ng_+i]-temp_mem[i], 0);
          }
        }
        ...
        // uout() << "l1_violation current iterate: " << l1_infeasibility_current_iterate << std::endl;
        // uout() << "current iterate l1 norm" << casadi_norm_1(nx_, d_nlp->z) << std::endl;
        ...
        double l1_infeasibility_trial_iterate = casadi_sum_viol(nx_+ng_, d->z_feas, d_nlp->lbz, d_nlp->ubz);
        double l_infty_infeasibility_trial_iterate = casadi_max_viol(nx_+ng_, d->z_feas, d_nlp->lbz, d_nlp->ubz);
        // uout() << "l1_violation current iterate: " << l1_infeasibility_trial_iterate << std::endl;
        double l1_infeasibility_linearized_step = casadi_sum(d->dx_restoration+nx_, 2*ng_);
        ...
        // uout() << "Step accepted variable: " << step_accepted << std::endl;

        if (step_accepted == 0 && m->primal_infeasibility <= tolerance_tube_beta_*tube_size){
          tube_size = tolerance_tube_beta_*tube_size;
        }

      } else {
        //%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
        // do the standard step update with feasibility iterations
        //%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        if (m->primal_infeasibility > tolerance_tube_beta_*tube_size) {
          ...
          double tr_ratio_phaseI = (m->primal_infeasibility - l_infty_infeasibility_trial_iterate) / m->primal_infeasibility;
          ...
          m_k = eval_m_k(mem);
          if (m->primal_infeasibility <= feasibility_tol_ && fabs(m_k) <= optimality_tol_) {
            if (print_status_)
              print("MESSAGE(tolerance-tube method): "
  ...
  void ToleranceTubeMethod::print_header() const {
    print("-------------------------------------------\n");
      print("This is casadi::Tolerance-Tube Method.\n");
      if (use_sqp_){
  ...
  void ToleranceTubeMethod::print_iteration() const {
    print("%4s %9s %14s %9s %9s %9s %9s %7s %5s %7s\n",
          "iter", "m_k", "objective", "tr_ratio", "inf_pr",
          "inf_du", "||d||", "lg(rg)", "tr_rad", "info");
  }

  void ToleranceTubeMethod::print_iteration(casadi_int iter, double obj,
  ...
  int ToleranceTubeMethod::solve_LP(ToleranceTubeMethodMemory* m, const double* g,
  ...
  int ToleranceTubeMethod::solve_restoration_LP(ToleranceTubeMethodMemory* m, const double* g,
                          const double* lbdz, const double* ubdz, const double* A,
                          double* x_opt, double* dlam) const {
    ScopedTiming tic(m->fstats.at("QP"));
    // Inputs
  ...
  int ToleranceTubeMethod::solve_QP(ToleranceTubeMethodMemory* m, const double* H, const double* g,
                           const double* lbdz, const double* ubdz, const double* A,
                           double* x_opt, double* dlam) const {
    ScopedTiming tic(m->fstats.at("QP"));
    // Inputs
...
void ToleranceTubeMethod::codegen_declarations(CodeGenerator& g) const {
  ...
  void ToleranceTubeMethod::codegen_body(CodeGenerator& g) const {
    g.add_auxiliary(CodeGenerator::AUX_TOLERANCETUBEMETHOD);
    ...
    g.local("d", "struct casadi_tolerancetubemethod_data");
    g.local("p", "struct casadi_tolerancetubemethod_prob");

    g << "d.prob = &p;\n";
    g << "p.sp_h = " << g.sparsity(Hsp_) << ";\n";
    g << "p.sp_a = " << g.sparsity(Asp_) << ";\n";
    g << "p.nlp = &p_nlp;\n";
    g << "casadi_tolerancetubemethod_init(&d, &iw, &w, " << sz_anderson_memory_ << ");\n";
    ...
    //   if(m->iter_count == 0) {
      ...
      // if (callback(m)) {
      //   if (print_status_) print("WARNING(tolerancetubemethod): Aborted by callback...\n");
      //   m->return_status = "User_Requested_Stop";
      //   break;
      // }
      ...
      g << "if (gain < 0) {\n";
      g << "if (" << print_status_ << ") {\n";
      g << "printf(\"WARNING(tolerancetubemethod): Indefinite Hessian detected\\n\");\n";
      g << "}\n";
      g << "}\n";
      g << "}\n";
      // g.comment("TBD");m
      ...
      codegen_feasibility_iterations(g, "tr_rad", "tube_size");
  ...
  void ToleranceTubeMethod::codegen_qp_solve(CodeGenerator& cg,
  ...
  void ToleranceTubeMethod::codegen_lp_solve(CodeGenerator& cg,
              const std::string& g,
              const std::string&  lbdz, const std::string& ubdz,
              const std::string&  A, const std::string& x_opt,
              const std::string&  dlam, int mode) const {
  ...
  void ToleranceTubeMethod::codegen_tr_update(CodeGenerator& cg,
  ...
  void ToleranceTubeMethod::codegen_eval_m_k(CodeGenerator& cg) const {
  // auto m = static_cast<ToleranceTubeMethodMemory*>(mem);
  // auto d = &m->d;
  // if (use_sqp_) {
  //   return 0.5*casadi_bilin(d->Bk, Hsp_, d->dx, d->dx) + casadi_dot(nx_, d->gf, d->dx);
  ...
  void ToleranceTubeMethod::codegen_eval_tr_ratio(CodeGenerator& cg,
      const std::string& val_f, const std::string& val_f_corr, const std::string& val_m_k) const {
    // return (val_f - val_f_corr) / (-val_m_k);
    cg << "tr_ratio = (" + val_f + "-" + val_f_corr + ") / (-" + val_m_k + ");\n";
  }

  void ToleranceTubeMethod::codegen_step_update(CodeGenerator& cg,
  ...
  void ToleranceTubeMethod::codegen_feasibility_iterations(CodeGenerator& cg,
    const std::string& tr_rad, const std::string& tube_size) const {
  ...
  cg << "for (int j=0;j<" << max_inner_iter_ << "; ++j) {\n";
    cg << "if (current_infeasibility < " << tube_size << ") {\n";
      cg << "inner_iter = j;\n";
    ...
    cg << "if (inner_iter % " << watchdog_ << "== 0) {\n";
      ...
      cg << "if (current_infeasibility < "<< tube_size << "&& projection_ratio < 0.5) {\n";
        cg << "ret = 0;\n";
        cg << "break; \n";
      cg << "}\n";
...
// }
  ...
  Dict ToleranceTubeMethod::get_stats(void* mem) const {
  ...
  ToleranceTubeMethod::ToleranceTubeMethod(DeserializingStream& s) : Nlpsol(s) {
    int version = s.version("ToleranceTubeMethod", 1, 3);
    s.unpack("ToleranceTubeMethod::qpsol", qpsol_standard_);

    s.unpack("ToleranceTubeMethod::exact_hessian", exact_hessian_);
    s.unpack("ToleranceTubeMethod::max_iter", max_iter_);
    s.unpack("ToleranceTubeMethod::min_iter", min_iter_);
    s.unpack("ToleranceTubeMethod::lbfgs_memory", lbfgs_memory_);
    s.unpack("ToleranceTubeMethod::tol_pr_", tol_pr_);
    s.unpack("ToleranceTubeMethod::tol_du_", tol_du_);
    s.unpack("ToleranceTubeMethod::print_header", print_header_);
    s.unpack("ToleranceTubeMethod::print_iteration", print_iteration_);
    s.unpack("ToleranceTubeMethod::print_status", print_status_);

    // if (version>=3) {
    //   s.unpack("ToleranceTubeMethod::elastic_mode", elastic_mode_);
    //   s.unpack("ToleranceTubeMethod::gamma_0", gamma_0_);
    //   s.unpack("ToleranceTubeMethod::gamma_max", gamma_max_);
    //   s.unpack("ToleranceTubeMethod::gamma_1_min", gamma_1_min_);
    //   s.unpack("ToleranceTubeMethod::init_feasible", init_feasible_);
    //   s.unpack("ToleranceTubeMethod::so_corr", so_corr_);
    // } else {
    //   // elastic_mode_ = false;
    //   // gamma_0_ = 0;
    //   // gamma_max_ = 0;
    ...
    s.unpack("ToleranceTubeMethod::Hsp", Hsp_);
    if (version==1) {
      Sparsity Hrsp;
      s.unpack("ToleranceTubeMethod::Hrsp", Hrsp);
    }
    s.unpack("ToleranceTubeMethod::Asp", Asp_);
    if (version==1) {
      double convexify_margin;
      s.unpack("ToleranceTubeMethod::convexify_margin", convexify_margin);
      char convexify_strategy;
      s.unpack("ToleranceTubeMethod::convexify_strategy", convexify_strategy);
      casadi_assert(convexify_strategy==0, "deserializtion failed.");
      bool Hsp_project;
      s.unpack("ToleranceTubeMethod::Hsp_project", Hsp_project);
      bool scc_transform;
      s.unpack("ToleranceTubeMethod::scc_transform", scc_transform);
      std::vector<casadi_int> scc_offset;
      s.unpack("ToleranceTubeMethod::scc_offset", scc_offset);
      std::vector<casadi_int> scc_mapping;
      s.unpack("ToleranceTubeMethod::scc_mapping", scc_mapping);
      casadi_int max_iter_eig;
      s.unpack("ToleranceTubeMethod::max_iter_eig", max_iter_eig);
      casadi_int block_size;
      s.unpack("ToleranceTubeMethod::block_size", block_size);
      Sparsity scc_sp;
      s.unpack("ToleranceTubeMethod::scc_sp", scc_sp);
      convexify_ = false;
    }
    if (version>=2) {
      s.unpack("ToleranceTubeMethod::convexify", convexify_);
      if (convexify_) Convexify::deserialize(s, "ToleranceTubeMethod::", convexify_data_);
    }
    set_tolerancetubemethod_prob();
  }

  void ToleranceTubeMethod::serialize_body(SerializingStream &s) const {
    Nlpsol::serialize_body(s);
    s.version("ToleranceTubeMethod", 3);
    s.pack("ToleranceTubeMethod::qpsol", qpsol_standard_);
    // s.pack("ToleranceTubeMethod::qpsol_standard_ela", qpsol_standard_ela_);
    s.pack("ToleranceTubeMethod::exact_hessian", exact_hessian_);
    s.pack("ToleranceTubeMethod::max_iter", max_iter_);
    s.pack("ToleranceTubeMethod::min_iter", min_iter_);
    s.pack("ToleranceTubeMethod::lbfgs_memory", lbfgs_memory_);
    s.pack("ToleranceTubeMethod::tol_pr_", tol_pr_);
    s.pack("ToleranceTubeMethod::tol_du_", tol_du_);
    s.pack("ToleranceTubeMethod::print_header", print_header_);
    s.pack("ToleranceTubeMethod::print_iteration", print_iteration_);
    s.pack("ToleranceTubeMethod::print_status", print_status_);

    s.pack("ToleranceTubeMethod::init_feasible", init_feasible_);
    s.pack("ToleranceTubeMethod::Hsp", Hsp_);
    s.pack("ToleranceTubeMethod::Asp", Asp_);
    s.pack("ToleranceTubeMethod::convexify", convexify_);
    if (convexify_) Convexify::serialize(s, "ToleranceTubeMethod::", convexify_data_);
  }
} // namespace casadi
